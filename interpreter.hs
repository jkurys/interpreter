...
data VarValue = IntVal Integer | BoolVal Bool | StrVal String | VoidVal | ArrVal [VarValue] | TupVal [VarValue] deriving Eq
...
instance Show VarValue where
    ...
    show (StrVal s) = s
    show VoidVal = ""
    show (TupVal tup) = "|" ++ go tup ++ "|" where
        go [el] = show el
        go (el:els) = show el ++ ", " ++ go els
    show (ArrVal arr) = "[" ++ go arr ++ "]" where
        go [el] = show el
        go (el:els) = show el ++ ", " ++ go els
...
getVariable name = do
    ...
    case loc of {
        Nothing -> throwRuntimeError ("Variable " ++ show name ++ " was not declared") >> return VoidVal;
        Just varLoc -> do
            let varType = Map.lookup varLoc s
            case varType of {
                Nothing -> throwRuntimeError "Unexpected error happened" >> return VoidVal;
                Just VoidVal -> throwRuntimeError ("Variable " ++ show name ++ " was not initialized") >> return VoidVal;
                Just t -> return t
            }
    }
...
replaceIndex :: Integer -> a -> [a] -> [a]
replaceIndex index newEl arr = a ++ (newEl:b) where (a, _:b) = splitAt (fromInteger index) arr
...
putReturnState :: Maybe VarValue -> InterpreterMonad ()
putReturnState newR = do
    IntState { varState = v, returnState = r, nextLoc = n, loopState = l } <- get
    put IntState { varState = v, returnState = newR, nextLoc = n, loopState = l }
...
evalExp (EMul _ e1 op e2) = do
    ...
    let IntVal n1 = e1'
    let IntVal n2 = e2'
    let (f, canFail) = case op of {
        (Times _) -> ((*), False);
        (Div _) -> (div, True);
        (Mod _) -> (mod, True)
    }
    CM.when (canFail && n2 == 0) (throwRuntimeError "Division or mod by zero")
    return $ IntVal (n1 `f` n2)
...
evalExp (EAcc _ arrName e) = do
    ...
    if index >= toInteger (length array) || index < 0
        then throwRuntimeError ( "Index " ++ show index ++ " is out of array " ++ show arrName ++ " bounds")
            >> return VoidVal
        else return $ array !! fromInteger index
...
evalExp (EAnd _ e1 e2) = evalExp e1
    >>= (\(BoolVal b1) -> evalExp e2 >>=
        (\(BoolVal b2) -> return $ BoolVal $ b1 && b2))
evalExp (EOr _ e1 e2) = evalExp e1 >>=
    (\(BoolVal b1) -> evalExp e2 >>=
        (\(BoolVal b2) -> return $ BoolVal $ b1 || b2))
evalExp (EApp _ fnName es) = do
    env <- askInt
    oldRet <- getReturnState
    ...
    return ret
evalExp (ETup _ es) = do
    result <- go es
    return $ TupVal result where
        go (e:es) = do
            e' <- evalExp e
            es' <- go es
            return (e':es')
...
execStmt (Ass _ (LArr _  name indexE) e) = do
    newVarVal <- evalExp e
    indexVal <- evalExp indexE
    let IntVal index = indexVal
    oldArr <- getVariable name
    let ArrVal array = oldArr
    CM.when (toInteger index >= toInteger (length array) || index < 0)
        $ throwRuntimeError ("Index " ++ show index ++ " is out of array " ++ show name ++ " bounds")
    let newArr = ArrVal $ replaceIndex index newVarVal array
    modifyVariable name newArr
...
execStmt (Ret _ e) = do
    e' <- evalExp e
    putReturnState $ Just e'
execStmt (VRet _) = do
    putReturnState $ Just VoidVal
execStmt (Break _) = do
...
execFunctionDecl :: Ident -> [Arg] -> Block -> Env -> InterpreterMonad Env
execFunctionDecl fnName args fnBody = declGlobal $ FnDef BNFC'NoPosition (Void BNFC'NoPosition) fnName args fnBody

execTupleDecl :: [Tup] -> [VarValue] -> Env -> InterpreterMonad Env
execTupleDecl ((ArgTup _ _ name):tups) (val:vals) env = declareVariable name val env >>= execTupleDecl tups vals
execTupleDecl ((NewTup _ tup):tups) ((TupVal tupValues):vals) env = execTupleDecl tup tupValues env >>= execTupleDecl tups vals
execTupleDecl _ _ env = return env
    ...
    CM.void $ go stmts env
        ...
        go (stmt:stmts) env = do
            case stmt of {
                ...
                DeclFn _ _ fnName args fnBody -> execFunctionDecl fnName args fnBody env  >>= (\env' -> localEnv (const env') (go stmts env'));
                DeclTup _ tup e -> do
                    result <- evalExp e
                    let TupVal vals = result
                    env' <- execTupleDecl tup vals env
                    localEnv (const env') (go stmts env')
                ;
                _ -> do
                    execStmt stmt
                    loopState <- getLoopState
                    returnState <- getReturnState
                    case (loopState, returnState) of {
                        (NormalLoop, Nothing) -> go stmts env;
                        _ -> return env
                    }
            }
        go [] env = return env
...
main = do
    ...
    case args of
        ...
        --UNINITIALIZED VARIABLE JEST TYLKO DO PRINTA